defmodule FountWorkshop.CharacterRewrite do
  ...
  def targets(%Screenplay{} = base, character_id, scene_ids) do
    with :ok <- validate_target_selection(base, character_id, scene_ids) do
      ids = character_dialogue_ids(base, character_id, scene_ids)
      if ids == [], do: {:error, :no_character_dialogue}, else: {:ok, ids}
    end
  end

  defp validate_target_selection(base, character_id, scene_ids) do
    cond do
  ...
  defp character_dialogue_ids(base, character_id, scene_ids) do
    speaker_cues =
      base.mentions
      |> Map.values()
      |> Enum.filter(
        &(&1.character_id == character_id and &1.role == :speaker_cue and &1.status == :confirmed)
      )
    ...
    |> Enum.flat_map(&scene_dialogue_ids(base, &1, speaker_cues))
    |> Enum.uniq()
  end

  defp scene_dialogue_ids(base, scene, speaker_cues) do
    members = MapSet.new(scene.element_ids)
    blocks = Enum.filter(base.ir.dialogue_blocks, &MapSet.member?(members, &1.cue_id))

    blocks
    |> Enum.with_index()
    |> Enum.flat_map(fn {block, index} ->
      responsive_dialogue_ids(base, blocks, index, block, speaker_cues)
    end)
  end

  defp responsive_dialogue_ids(base, blocks, index, block, speaker_cues) do
  ...
  def run(repo, key, character_id, scene_ids, direction, client) do
    with {:ok, base} <- Persistence.load(repo, key),
      ...
      run_character_rewrite(repo, base, session, character_id, scene_ids, direction, client, ids)
    end
  end

  defp run_character_rewrite(repo, base, session, character_id, scene_ids, direction, client, ids) do
