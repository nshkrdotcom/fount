defmodule Fount.LiveArtifacts do
  ...
  def require!({:error, _, session}) when is_map(session), do: throw({:live_failure, %{"session_id" => session["id"], "progress" => session["progress"]}})
  ...
  defp finish(directory, mode, started, status, result) do
    files = Path.wildcard(Path.join(directory, "**/*")) |> Enum.filter(&File.regular?/1)
      |> Enum.map(fn path -> %{"path" => Path.relative_to(path, directory), "bytes" => File.stat!(path).size,
        "sha256" => :crypto.hash(:sha256, File.read!(path)) |> Base.encode16(case: :lower)} end)
    write!(directory, "run.json", %{"mode" => mode, "status" => status, "completed_at" => DateTime.to_iso8601(DateTime.utc_now()),
      "elapsed_ms" => System.monotonic_time(:millisecond) - started, "result" => result, "files" => files,
      "services" => "real-only; see result/report provenance for actual calls", "verification" => "This file is produced only when the local example is actually run."})
  end
end
