defmodule FountProbe.State do
  ...
  def speaker_before(model, character_id, scene_id),
    do: view(model, scene_id, "character_access", character_id: character_id)
  ...
  defp view(model, scene_id, projection, opts) do
    ...
    with {:ok, state, evidence} <- FountProbe.Projection.at(model, point, projection, opts) do
      ...
      scenes =
        evidence
        |> Enum.group_by(fn e ->
          s = Fount.Query.scene_for(model, e["target"]["id"])
          s && s.id
        end)

      ordered =
        model.ir.scenes
        |> Enum.filter(&Map.has_key?(scenes, &1.id))
        |> Enum.map(fn s ->
          %{
            "id" => s.id,
            "elements" =>
              Enum.map(scenes[s.id], fn e ->
                %{
                  "id" => e["target"]["id"],
                  "text" => by_evidence[e["evidence_id"]]["text"],
                  "type" => by_evidence[e["evidence_id"]]["type"]
                }
              end)
          }
        end)
      ...
      {:ok,
       Map.merge(state, %{
         ...
         "scenes" => ordered,
         "scene_ids" => Enum.map(ordered, & &1["id"])
       })}
    end
  end
end
