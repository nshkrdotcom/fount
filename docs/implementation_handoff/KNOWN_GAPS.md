...
**Current scope change:** this is the original source-only gap list, not the current implementation queue. Feature additions are frozen and mapped to [FUTURE_DEVELOPMENT.md](FUTURE_DEVELOPMENT.md); current stability evidence and blockers are in [STABILITY_CERTIFICATION.md](STABILITY_CERTIFICATION.md). The original requirements below remain preserved for later development.
