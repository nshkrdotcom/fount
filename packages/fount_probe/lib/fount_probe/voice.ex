      ...
      with {:ok, units} <- Projection.select(model, params["selection"]) do
          ...
          Enum.flat_map(ids, fn id ->
            ...
            |> Enum.map(fn b ->
              selected =
                Enum.filter(
                  units,
                  &(&1["target"]["id"] in b.body_ids and &1["type"] == "dialogue")
                )
        ...
        profiles =
          Map.new(ids, fn id ->
            ...
            selected =
              if is_list(params["training_targets"]),
                do:
                  Enum.filter(samples, fn t ->
                    t.id in Enum.map(params["training_targets"], & &1["id"])
                  end),
                else: evenly(samples, min(8, max(length(samples) - 3, 0)))

            chosen =
              if is_binary(explicit) and String.trim(explicit) != "", do: [], else: selected
    ...
    with {:ok, prepared} <- prepare(model, params) do
      if prepared.insufficient != [] do
        ...
        with {:ok, result} <-
          ...
          groups =
            Map.new(params["comparison_groups"] || [], fn group ->
           ...
           Report.new(model, "voice", params, %{
             ...
             data:
               Map.merge(stats, %{
                 ...
                 "groups" => groups,
                 "profiles" => Fount.Screenplay.Model.plain(prepared.profiles),
  ...
  def aggregate(tests, entries, labels) do
    ...
    rows =
      Map.new(labels, fn label ->
        ...
        answers =
          Enum.flat_map(actual, fn t ->
            case get_in(by_id, [t.id, "answers", "voice"]) do
              %{"probabilities" => p} = a when is_map(p) -> [a]
              _ -> []
            end
          end)
