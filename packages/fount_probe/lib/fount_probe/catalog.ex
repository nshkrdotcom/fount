defmodule FountProbe.Catalog do
  ...
  @catalog %{
    ...
    "knowledge_trace" => {~w(proposition subjects points), ~w(access_mode suspicion behavior_element_ids intended_reveal_point)},
    "locate_boundary" => {~w(scene_id proposition projection), ~w(character_id threshold include_prior_context)},
    "dependencies" => {~w(targets selection), ~w(record_report_ids include_alternative_support inspect_setup_purpose)},
  ...
  def tools do
    @catalog |> Enum.sort() |> Enum.map(fn {name, {required, optional}} -> %{"name" => name, "version" => 1, "required" => required, "optional" => optional,
      "input_schema" => schema(name), "result" => "FountProbe.Report v1", "writes_screenplay" => false} end)
  end
  def names, do: Map.keys(@catalog) |> Enum.sort()
  def schema(name) do
    {required, optional} = Map.fetch!(@catalog, name)
    properties = Map.new(required ++ optional, fn key ->
      s = cond do
        key in @lists -> %{"type" => "array"}
        key in @booleans -> %{"type" => "boolean"}
        key in @strings -> %{"type" => "string", "minLength" => 1}
        key in ["limit", "adjacent_scenes"] -> %{"type" => "integer", "minimum" => 0, "maximum" => 500}
        key == "threshold" -> %{"type" => "number", "minimum" => 0, "maximum" => 1}
        true -> %{"type" => "object"}
      end
      {key, s}
    end)
  ...
  def validate(model, name, params) when is_map(params) do
    if Map.has_key?(@catalog, name) do
      with :ok <- Fount.Writing.Schema.validate(schema(name), params),
           :ok <- selection(model, params), :ok <- groups(model, params), :ok <- ids(model, params), :ok <- domain(name, params) do :ok end
    else {:error, :unknown_probe_tool} end
  end
  def validate(_, _, _), do: {:error, :invalid_probe_parameters}
  defp selection(model, %{"selection" => s}) do
    case FountProbe.Projection.selected_ids(model, s) do {:ok, _} -> :ok; error -> error end
  end
  ...
  defp groups(model, %{"comparison_groups" => groups}) do
    Enum.reduce_while(groups, {:ok, MapSet.new(), MapSet.new()}, fn
      ...
      _, _ -> {:halt, {:error, :invalid_comparison_group}}
    end)
    |> case do
      {:ok, _, _} -> :ok
      error -> error
  ...
  defp ids(model, params) do
    ...
    points = Map.get(params, "points", []) ++ List.wrap(params["point"]) ++ List.wrap(params["intended_reveal_point"])
    cond do
      Enum.any?(scene_ids, &is_nil(Fount.Query.scene(model, &1))) -> {:error, :unknown_scene}
      Enum.any?(characters, &(not Map.has_key?(model.cast, &1))) -> {:error, :unknown_character}
      Enum.any?(points, &(not match?({:ok, _}, FountProbe.Projection.cutoff(model, &1)))) -> {:error, :illegal_point}
      true -> :ok
    end
  end
  defp domain("search", p) do
    filters = Map.get(p, "filters", %{})
    valid = (Map.keys(filters) -- ~w(scene_ids character_ids character_role element_types location authored_collection_ids include_omitted include_notes include_boneyards)) == [] and
      Map.get(p, "mode", "retrieve") in ~w(retrieve inspect_all) and Map.get(filters, "character_role", "speaker") in ~w(speaker reference declared_present)
    if valid, do: :ok, else: {:error, :invalid_search_filter}
  end
  defp domain(name, p) when name in ["locate_boundary", "ablate"] do
    if p["projection"] in ~w(page_reader audience_estimate character_access) and (p["projection"] != "character_access" or is_binary(p["character_id"])), do: :ok, else: {:error, :invalid_projection}
  end
  defp domain("knowledge_trace", p) do
    if p["points"] != [] and p["subjects"] != [] and Map.get(p, "access_mode", "evidence") in ~w(evidence writer_declared), do: :ok, else: {:error, :empty_or_invalid_trace}
  end
  defp domain("dialogue", p) do
    allowed = ~w(repetition exposition subtext tactic responsiveness rhythm knowledge voice)
    if p["lenses"] != [] and Enum.all?(p["lenses"], &(&1 in allowed)), do: :ok, else: {:error, :unknown_dialogue_lens}
  end
  defp domain("voice", p), do: if(length(p["character_ids"]) in 2..8, do: :ok, else: {:error, :voice_requires_two_to_eight_characters})
  defp domain("scene_lift", p), do: if(p["scene_ids"] != [], do: :ok, else: {:error, :empty_scene_lift})
