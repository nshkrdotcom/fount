defmodule FountWorkshop.Store do
  ...
  @actions [:save_session, :session, :save_candidate, :candidate, :candidates_for_session,
    :load_revision, :at_revision, :load, :create, :save_report, :report, :history,
  ...
  def normalize(map) when is_map(map), do: Map.new(map, fn {key, value} -> {to_string(key), value} end)
