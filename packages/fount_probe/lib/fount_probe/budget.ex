defmodule FountProbe.Budget do
  ...
  def new(opts \\ []) do
    ...
    %{counter: counter, limits: %{inference: Keyword.get(opts, :max_inference_calls, 12), jev_states: Keyword.get(opts, :max_jev_states, 500)}}
  end
  ...
  def snapshot(budget), do: %{"inference" => :atomics.get(budget.counter, 1), "jev_states" => :atomics.get(budget.counter, 2)}
end
