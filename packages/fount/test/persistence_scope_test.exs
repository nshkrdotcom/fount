defmodule Fount.PersistenceScopeTest do
  ...
  test "all projection schemas carry the immutable revision key" do
    for module <- [Schema.Scene, Schema.Element, Schema.Character, Schema.DialogueBlock, Schema.AuthoredItem, Schema.Mention] do
  ...
  test "core review gate prevents direct persistence callers bypassing hard pins" do
    id = Fount.ID.v4(); base = Fount.ID.v4()
    candidate = %{"id" => id, "base_revision_id" => base, "content_hash" => "hash", "report_ids" => [], "checks" => [%{"constraint_id" => "pin", "severity" => "required", "status" => "fail", "evaluation" => "deterministic"}]}
    review = %{"candidate_id" => id, "content_hash" => "hash", "actor" => "writer", "report_ids" => [], "overrides" => [%{"constraint_id" => "pin", "reason" => "please"}]}
