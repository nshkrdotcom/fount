defmodule Fount.CLI do
  ...
  def run(command, argv) do
    with {:ok, opts, args} <- S.parse(argv, @flags) do
      if opts[:help] do
        ...
        with :ok <- S.required(opts, [:key]), :ok <- arguments(command, args, opts),
             {:ok, repo} <- S.connect() do
  ...
  defp dispatch("import", [path], opts, repo) do
    with {:ok, bytes} <- File.read(path),
         ...
         {:ok, root} <- Fount.Persistence.create(repo, opts[:key], model, provenance: %{"import_file" => Path.basename(path), "diagnostics" => Fount.Screenplay.Model.plain(diagnostics)}) do
      {:ok, summary(root)}
    end
  end
  defp dispatch("inspect", [], opts, repo) do
    with {:ok, model} <- S.load(repo, opts) do
      {:ok, Map.merge(summary(model), %{"scenes" => Fount.Screenplay.Model.plain(model.ir.scenes),
        "elements" => Fount.Screenplay.Model.plain(model.ir.elements), "characters" => Fount.Screenplay.Model.plain(Map.values(model.cast)),
        "authored_items" => model.authored_items, "dialogue_blocks" => Fount.Screenplay.Model.plain(model.ir.dialogue_blocks)})}
    end
  end
  defp dispatch("export", [], opts, repo) do
    with {:ok, model} <- S.load(repo, opts), {:ok, result} <- Fount.Interchange.write(model, opts[:format], include_source: true),
         :ok <- File.mkdir_p(Path.dirname(opts[:output])), :ok <- File.write(opts[:output], result.data),
  ...
  def summary(model), do: %{"screenplay_id" => model.id, "revision_id" => model.revision.id,
    "content_hash" => model.revision.content_hash, "scenes" => length(model.ir.scenes), "elements" => length(model.ir.elements)}
