defmodule FountProbe.ContinuationConstraintsTest do
  ...
  test "a shifted protected fragment is mapped, but ambiguous copies are rejected" do
    base =
      Fount.Screenplay.new(
        scenes: [
          %{heading: "INT. SHED - DAY", elements: [%{type: :action, text: "She keeps the key."}]}
        ]
      )
    ...
    {:ok, next, _} =
      Fount.Screenplay.apply(base, [
        %{
          "kind" => "replace_text",
          "target" => c["target"],
          "value" => "Quietly, she keeps the key."
        }
      ])

    [result] = Constraints.deterministic(base, next, [c])
    assert result["status"] == "pass"
    assert result["measurements"]["candidate_span"]["byte_start"] == 19

    {:ok, bad, _} =
      Fount.Screenplay.apply(base, [
        %{"kind" => "replace_text", "target" => c["target"], "value" => "the key and the key"}
      ])
  ...
  test "negative semantic intent uses the complementary probability" do
    constraint = %{
      "kind" => "semantic",
      "spec" => %{"question_type" => "noul", "expected" => false}
    }
  ...
  test "missing external pagination never becomes zero or a pass" do
    ...
    assert [%{"status" => "unknown", "evaluation" => "external"}] =
             Constraints.deterministic(base, base, [c])
  end
end
