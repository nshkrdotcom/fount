defmodule FountWorkshop.ReviewExport do
  ...
  def export(session_id, directory, services, opts \\ []) do
    with {:ok, session} <- Session.get(session_id, services),
      ...
      File.write!(
        Path.join(directory, "base.fountain"),
        Fount.Screenplay.to_fountain(base, mode: :spec)
      )

      results =
        Enum.map(session["candidates"], fn candidate ->
          export_candidate(base, candidate, directory, services, opts)
        end)
      ...
      manifest = %{
        ...
        "files" =>
          ...
          |> Enum.map(fn path ->
            %{"path" => Path.basename(path), "sha256" => hash(File.read!(path))}
          end)
      }
  ...
  defp export_candidate(base, candidate, directory, services, opts) do
    ...
    reports =
      Enum.map(report_ids, fn report_id ->
        case Store.call(services[:store], :report, [report_id]) do
          {:ok, report} -> report["payload"]
          {:error, _} -> %{"id" => report_id, "status" => "unavailable"}
        end
      end)
      ...
      "source_diff" =>
        Enum.map(
          String.myers_difference(
            Fount.Screenplay.to_fountain(base),
            Fount.Screenplay.to_fountain(model)
          ),
    ...
    %{
      ...
      "pdf" => result(pdf),
      "page_delta" =>
        if(layout && layout["data"]["same_settings"],
          do: layout["data"]["candidate_pages"] - layout["data"]["base_pages"],
          else: nil
        ),
      "status_note" =>
  ...
  defp result({:error, reason}),
    do: %{"status" => "failed", "reason" => inspect(reason, limit: 10, printable_limit: 500)}

  defp write!(dir, name, value),
    do:
      File.write!(
        Path.join(dir, name),
