defmodule FountProbe.ContinuationVoiceTest do
  ...
  test "blind preparation removes names and keeps training and test text disjoint" do
    scenes =
      for n <- 1..12,
          do: %{
            ...
            elements: [
              %{type: :character, text: if(rem(n, 2) == 0, do: "MARA", else: "DAN")},
              %{type: :dialogue, text: "Mara, I brought the numbered parcel #{n} here today."}
            ]
          }
    ...
    assert {:ok, prepared} =
             FountProbe.Voice.prepare(model, %{
               "character_ids" => ids,
               "selection" => %{"whole_screenplay" => true}
             })

    assert Enum.all?(prepared.tests, fn t ->
             t.text not in Enum.flat_map(prepared.profiles, fn {_, p} -> p.samples end)
           end)
