defmodule FountProbe.Jev do
  ...
  def evaluate(client, inputs, questions, opts \\ []) do
    with {:ok, questions, profile} <- FountProbe.Profile.compile(questions, Keyword.get(opts, :profile_id)) do
  ...
  defp evaluate_compiled(client, inputs, questions, profile, opts) do
    ...
    {eligible, oversize} = Enum.split_with(scheduled, &(byte_size(CanonicalJSON.encode!(&1["state"])) <= max_bytes))
    rejected = Enum.map(pending, &error(&1["id"], "state_limit")) ++ Enum.map(oversize, &error(&1["id"], "context_limit"))
    ...
    requests = Enum.map(eligible, &%{"id" => &1["id"], "state" => CanonicalJSON.encode!(&1["state"])})
    result = cond do
      requests == [] -> {:ok, %{"entries" => [], "errors" => [], "status" => "complete"}}
      is_nil(client) -> {:error, :missing_system_one_client}
      true -> Executor.evaluate(client, requests, questions, Keyword.take(opts, [:max_concurrency, :max_pending, :task_timeout_ms, :attempt_timeout_ms]))
    end
    case result do
      ...
      {:ok, batch} ->
        ...
        {:ok, %{"entries" => ordered, "status" => if(Enum.all?(ordered, &(&1["status"] == "complete")), do: "complete", else: "partial"),
          "errors" => batch["errors"] || [], "requested" => length(inputs), "scheduled" => length(eligible),
          "prepared_fingerprint" => batch["prepared_fingerprint"], "elapsed_ms" => batch["elapsed_ms"],
          "question_profile_sha256" => CanonicalJSON.hash(question_profile(questions)),
          "question_profile" => question_profile(questions), "profile_asset" => profile,
  ...
  defp decode_entry(%{"input_id" => id, "result" => {:ok, response}}, questions) do
    ...
    decoded = Map.new(questions, fn {key, _} ->
    ...
    %{"input_id" => id, "status" => if(valid, do: "complete", else: "partial"), "answers" => decoded,
      "provenance" => Fount.Screenplay.Model.plain(Map.take(response, [:model, :usage, :request_id, :prepared_fingerprint]))}
  end
  defp decode_entry(%{"input_id" => id, "result" => {:error, reason}}, _), do: error(id, safe_reason(reason))
  ...
  def answer(%SystemOneSDK.ScoreAnswer{} = a) do
    ...
    with true <- is_number(a.score),
         {:ok, _} <- DecisionPolicy.semantic_distribution(probabilities, Map.keys(probabilities), a.confidence),
         true <- abs(Enum.sum(Map.values(probabilities)) - 1.0) <= 0.02 do
      %{"type" => "score", "score" => a.score, "probabilities" => probabilities,
        "confidence" => a.confidence, "status" => "complete", "rubric" => Fount.Screenplay.Model.plain(a.rubric)}
    else
      _ -> %{"status" => "error", "reason" => "invalid_score_distribution"}
    end
  end
  def answer(_), do: %{"status" => "error", "reason" => "missing_or_unknown_answer"}
  defp error(id, reason), do: %{"input_id" => id, "status" => "error", "error" => reason, "answers" => %{}}
