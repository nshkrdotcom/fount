defmodule FountProbe.Dialogue do
  ...
  def run(model, params, clients, opts \\ []) do
    with {:ok, units} <- Projection.select(model, params["selection"]),
      ...
      turns = dialogue_turns(model, lines, params)

      counts = dialogue_counts(turns)

      duplicates = duplicate_turns(turns)
  ...
  defp turn_contexts(model, turns, whole) do
  ...
  defp dialogue_turns(model, lines, params) do
