...
defmodule FountWorkshop.SessionContinuationTest do
  ...
  test "partial materialization retains pages; resume skips saved branches and reuses inspected preparation" do
    ...
    {:ok, script} = Agent.start_link(fn -> [fn _ -> strategies() end, fn _ -> proposal(model, "a", "Mara blocks the closing door.") end, {:error, :temporary_failure}] end)
    on_exit(fn -> if Process.alive?(store), do: Agent.stop(store); if Process.alive?(script), do: Agent.stop(script) end)
    ...
    request = %{"version" => 1, "workflow" => "develop", "mode" => "revise", "base_revision_id" => model.revision.id,
      "instruction" => "A writer must stop a closing door without admitting why.", "selection" => %{"whole_screenplay" => true},
      "constraints" => [], "alternatives" => 2, "options" => %{"placement" => %{"kind" => "start"}}}
    ...
    Agent.update(script, fn [] -> [fn _ -> proposal(model, "b", "Mara wedges her shoe into the door.") end] end)
  ...
  defp strategies do
    %{"strategies" => Enum.map(["a", "b"], fn id ->
      %{"id" => id, "title" => id, "premise_of_change" => "Opposing the closure", "dramatic_mechanism" => "A visible choice", "entry_state" => "Outside", "exit_state" => "Inside",
        "beats" => ["Stop the door"], "preserves" => [], "changes" => [], "inventions" => [], "consequences" => [], "evidence_ids" => [], "open_questions" => []}
    end)}
  end
  defp proposal(model, strategy, text) do
    %{"version" => 1, "base_revision_id" => model.revision.id, "strategy_id" => strategy, "summary" => text, "inventions" => [], "unresolved_questions" => [],
      "groups" => [%{"id" => "opening", "title" => "Opening", "reason" => "A played choice", "depends_on" => [], "addresses_notes" => [], "evidence_ids" => [], "origin" => "generated_text",
        "operations" => [%{"kind" => "insert_scene", "value" => %{"after_scene_id" => nil, "scene" => %{"local_id" => "new:opening", "heading" => "INT. OFFICE - NIGHT",
          "elements" => [%{"local_id" => "new:action", "type" => "action", "text" => text, "attrs" => %{}}]}}}]}]}
  end
end
