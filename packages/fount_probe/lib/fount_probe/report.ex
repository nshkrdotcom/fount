defmodule FountProbe.Report do
  ...
  def new(model, tool, request, attrs \\ %{}) do
    ...
    profile = %{
      ...
      "identity_scope" =>
        if(hashes == [],
          do: "deterministic_tool_version_or_completion_trace",
          else: "effective_question_definitions"
        )
    }
    ...
    struct(
      ...
      Map.merge(
        %{
          ...
          primary_revision_id: model.revision.id,
          source_revision_ids: [model.revision.id],
          request: request,
          profile: profile
        },
        attrs
  ...
  def persistence(report, session_id \\ nil) do
    ...
    payload =
      Map.put(
        payload,
        "citations",
        citations(Map.take(payload, ~w(findings data annotations graph)))
      )

    %{
      id: report.id,
      screenplay_id: report.screenplay_id,
      primary_revision_id: report.primary_revision_id,
      source_revision_ids: report.source_revision_ids,
  ...
  defp citations(value) when is_list(value),
    do: value |> Enum.flat_map(&citations/1) |> Enum.uniq()

  defp citations(_), do: []

  def failure(model, tool, params, reason),
    do:
      new(model, tool || "invalid_request", params, %{
        status: "failed",
        errors: [%{"code" => "request_failed", "message" => inspect(reason, limit: 20)}]
      })
end
