defmodule Fount.Writing.LocalReferences do
  ...
  def compile(value, opts \\ []) do
    try do
      ...
      if length(labels) != length(Enum.uniq(labels)), do: fail(:duplicate_local_id)
      given = Keyword.get(opts, :reference_map, %{})
      generator = Keyword.get(opts, :uuid_generator, &Fount.ID.v4/0)
      ids = Map.new(labels, fn label -> {label, Map.get_lazy(given, label, generator)} end)
      if Enum.any?(ids, fn {_, id} -> not is_binary(id) or not Regex.match?(@uuid, id) end), do: fail(:invalid_allocated_uuid)
      if length(Enum.uniq(Map.values(ids))) != map_size(ids), do: fail(:duplicate_allocated_uuid)
      ids = Enum.reduce(declarations, ids, fn d, acc ->
  ...
  def namespace(value, prefix) when is_binary(prefix) do
    names = value |> declarations() |> Map.new(fn d ->
      "new:" <> label = d["local_id"]
      {d["local_id"], "new:" <> prefix <> "-" <> label}
    end)
    names = Enum.reduce(names, names, fn {"new:" <> label, "new:" <> renamed}, acc -> Map.put(acc, "new:block-" <> label, "new:block-" <> renamed) end)
    rename(value, names)
  end
  ...
  defp declarations(map) when is_map(map) do
    own = if Map.has_key?(map, "local_id") do
      local = map["local_id"]
      if not is_binary(local) or not Regex.match?(@label, local) or Map.has_key?(map, "id"), do: fail(:invalid_local_id)
      [map]
    else
      []
    end
