defmodule FountWorkshop.NoteResponse do
  ...
  def propose(%Screenplay{} = base, note_id, target_ids, client) do
    ...
    cond do
      ...
      true ->
        propose_note(base, note, note_id, target_ids, client)
    end
  end

  defp propose_note(base, note, note_id, target_ids, client) do
    note_scene = Query.scene_for(base, note_id)

    if note_scene == nil or Enum.any?(target_ids, &outside_note_scene?(base, &1, note_scene)) do
  ...
  def run(repo, key, note_id, target_ids, client) do
    with {:ok, base} <- Persistence.load(repo, key),
      ...
      run_note_response(repo, base, session, note_id, target_ids, client)
    end
  end

  defp run_note_response(repo, base, session, note_id, target_ids, client) do
  ...
  defp outside_note_scene?(base, id, note_scene) do
