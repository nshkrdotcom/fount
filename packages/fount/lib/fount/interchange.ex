defmodule Fount.Interchange do
  ...
  def write(model, "fountain", opts) do
    with {:ok, result} <- Fount.Screenplay.export_fountain(model, opts) do
      {:ok,
       %{
         result
         | losses: Enum.uniq(result.losses ++ identity_losses(model)),
           metadata: %{fidelity: "screenplay_text_not_authoring_database"}
       }}
    end
  end
    ...
    with {:ok, result} <- Fount.Screenplay.to_fdx(model) do
      {:ok,
       %{
         result
         | losses: Enum.uniq(result.losses ++ identity_losses(model)),
           metadata: %{fidelity: "supported_final_draft_subset"}
       }}
    end
  end
  ...
  def matrix do
    %{
      "fountain" => %{
        "unchanged_import_bytes" => "preserved",
        "edited_text" => "canonical_regeneration",
        "stable_ids" => false,
        "authored_items" => false,
        "revision_history" => false
      },
      "fdx" => %{
        "unchanged_import_bytes" => "preserved",
        "edited_text" => "supported_subset_with_losses",
        "stable_ids" => false,
        "authored_items" => false,
        "production_roundtrip" => "not_claimed"
      },
      "json" => %{
  ...
  defp identity_losses(model) do
    ["Fount UUIDs, revision lineage and exact evidence identities are not encoded in this text format"] ++
      if(map_size(model.authored_items) > 0,
        do: [
          "Writer-authored items remain in PostgreSQL or canonical JSON; they are not flattened into screenplay pages"
        ],
        else: []
      ) ++
      if(map_size(model.cast) > 0,
        do: ["Confirmed cast and mention identities become literal screenplay text"],
        else: []
      )
  end
end
