defmodule FountWorkshop.Candidate do
  ...
  def compile(base, proposal, opts \\ []) do
    with :ok <- Schema.validate("proposal.schema.json", proposal),
         true <-
           proposal["base_revision_id"] == base.revision.id or {:error, :proposal_base_mismatch},
      ...
      checks =
        FountProbe.Constraints.deterministic(
          ...
          draft,
          constraints,
          Keyword.put(opts, :inventions, proposal["inventions"])
        )

      provenance = %{
        "proposal" => proposal,
        "allocated_ids" => changes.local_references,
        "compiled_operations" =>
          case LocalReferences.compile(operations,
                 reference_map: changes.local_references,
      ...
      {:ok,
       %{
         ...
         "screenplay" => draft,
         "status" => "open",
         "label" => Keyword.get(opts, :label, proposal["summary"]),
         "strategy" => Keyword.get(opts, :strategy, %{}),
         "change_groups" => groups,
         "lineage" => Keyword.get(opts, :lineage, []),
         "parent_candidate_id" => Keyword.get(opts, :parent_candidate_id),
  ...
  def select(id, groups, services, opts) when is_binary(id),
    do: FountWorkshop.CandidateAPI.select(id, groups, services, opts)
  ...
  def edit(id, operations, services, opts) when is_binary(id),
    do: FountWorkshop.CandidateAPI.edit(id, operations, services, opts)

  def edit(%Fount.Screenplay{} = base, candidate, operations, actor),
    do: edit(base, candidate, operations, actor, [])
  ...
  def edit(base, candidate, operations, actor, opts) do
    if not is_binary(actor) or String.trim(actor) == "" do
      ...
      group = %{
        "id" => "writer-" <> ID.v4(),
        "title" => "Writer edit",
        "reason" => "Explicit writer revision by " <> actor,
        "depends_on" => Enum.map(candidate["change_groups"], & &1["id"]),
        "addresses_notes" => [],
        "evidence_ids" => [],
        "operations" => ops,
        "origin" => "writer_edit"
      ...
      compile(
        ...
        |> Keyword.put(:parent_candidate_id, candidate["id"])
        |> Keyword.put(:lineage, [
          %{"candidate_id" => candidate["id"], "operation" => "writer_edit", "actor" => actor}
        ])
      )
    end
  end
  ...
  def combine(ids, selection, services, opts) when is_list(ids),
    do: FountWorkshop.CandidateAPI.combine(ids, selection, services, opts)

  def combine(%Fount.Screenplay{} = base, candidates, request, opts) do
    by_id = Map.new(candidates, &{&1["id"], &1})

    with true <-
           (is_list(request["picks"]) and request["picks"] != []) or {:error, :empty_combination},
         true <-
           Enum.all?(
             candidates,
             &(&1["base_revision_id"] == base.revision.id and &1["screenplay_id"] == base.id)
           ) or {:error, :combination_base_mismatch},
      ...
      p = %{
        ...
        "base_revision_id" => base.revision.id,
        "strategy_id" => nil,
        "summary" => Map.get(request, "label", "Writer-selected combination"),
        "groups" => chosen,
        "inventions" => Enum.flat_map(candidates, &proposal(&1)["inventions"]),
        "unresolved_questions" => []
      }

      evidence =
        candidates
        |> Enum.flat_map(&(&1["provenance"]["evidence"] || []))
  ...
  defp range_groups(base, source, ranges) when is_list(ranges) and ranges != [] do
    Enum.reduce_while(Enum.with_index(ranges), {:ok, []}, fn {range, n}, {:ok, groups} ->
      with %{"source" => %{"kind" => "element"} = from, "target" => %{"kind" => "element"} = to} <-
             range,
        ...
        group = %{
          ...
          "reason" => "Writer-selected exact source range",
          "depends_on" => [],
          "addresses_notes" => [],
          "evidence_ids" => [],
          "operations" => [op],
        ...
        _ -> {:halt, {:error, :invalid_combination_range}}
      end
    end)
  end
  ...
  defp choose_overlaps(base, groups, choices) do
    ...
    cond do
      not is_list(choices) or Enum.any?(choices, &(&1 not in ids)) ->
        {:error, :invalid_overlap_choice}

      overlaps == [] ->
        {:ok, groups}

      Enum.any?(overlaps, fn o -> Enum.count([o["left"], o["right"]], &(&1 in choices)) != 1 end) ->
        {:error, {:overlapping_selections, overlaps}}

      true ->
        losers =
          overlaps
          |> Enum.flat_map(fn o -> Enum.reject([o["left"], o["right"]], &(&1 in choices)) end)
          |> MapSet.new()
  ...
  def check(base, candidate, services, opts \\ []) do
    ...
    with {:ok, report} <-
      ...
      report = %{
        report
        | errors: report.errors ++ layout_errors,
          status: if(layout_errors == [], do: report.status, else: "partial")
      }
  ...
  def rebase(id, current, resolutions, services) when is_binary(id),
    do: FountWorkshop.Rebase.run(id, current, resolutions, services)
  ...
  def rebase(candidate, current, request, services, opts) do
    if candidate["screenplay_id"] != current.id do
      ...
      context = %{
        ...
        "previous_base_revision_id" => candidate["base_revision_id"],
        "previous_candidate_pages" => Screenplay.to_fountain(candidate["screenplay"]),
        "previous_proposal" => proposal(candidate)
      }
      ...
      FountWorkshop.Session.start(
        ...
        request,
        services,
        Keyword.put(opts, :rebase_context, context)
      )
    end
  end
  ...
  defp inherited(c, opts),
    do:
      opts
      |> Keyword.put_new(
        :writer_edit,
        Enum.any?(c["change_groups"], &(&1["origin"] in ["writer_edit", "mixed"]))
      )
      |> Keyword.put_new(:reference_map, c["provenance"]["allocated_ids"] || %{})
  ...
  defp origins(groups, opts) do
    if Enum.any?(groups, &(&1["origin"] in ["writer_edit", "mixed"])) and
         not Keyword.get(opts, :writer_edit, false),
       do: {:error, :model_cannot_claim_writer_origin},
       else: :ok
  end
  ...
  defp placement(base, draft, %{
         "kind" => "between_scenes",
         "after_scene_id" => left,
         "before_scene_id" => right
       }) do
    a = Fount.Query.scene(draft, left)
    b = Fount.Query.scene(draft, right)
    ids = Enum.map(draft.ir.scenes, & &1.id)

    unchanged =
      a && b &&
        Enum.all?([left, right], fn id ->
          s = Fount.Query.scene(base, id)

          s.element_ids == Fount.Query.scene(draft, id).element_ids and
            Enum.all?(
              s.element_ids,
              &(Fount.Query.node(base, &1).text == Fount.Query.node(draft, &1).text)
            )
        end)

    if unchanged and
         Enum.find_index(ids, &(&1 == right)) - Enum.find_index(ids, &(&1 == left)) > 1,
       do: :ok,
       else: {:error, :bridge_must_retain_both_neighbors_and_add_pages}
  end
    ...
    case Keyword.get(opts, :target_scene_count) do
      ...
      target ->
        ...
        if result_count == target,
          do: :ok,
          else: {:error, {:wrong_rebuilt_scene_count, target, result_count}}
    end
  end
end
